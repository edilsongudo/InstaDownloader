...
def long_operation_thread():
    ...
    def baixardriver():
        if os.path.isfile('/Program Files/InstagramDownloader/geckodriver.exe') == False:

                r = requests.get('https://github.com/mozilla/geckodriver/releases/download/v0.26.0/geckodriver-v0.26.0-win32.zip')

                with open('/Program Files/InstagramDownloader/gecko.zip', 'wb') as f:
                    f.write(r.content)

                shutil.unpack_archive('/Program Files/InstagramDownloader/gecko.zip', '/Program Files/InstagramDownloader/')

    baixardriver()
    browser_launched = False

    while not browser_launched:
        try:
            print('Lauching...')
            options = Options()
            options.headless = True
            browser = webdriver.Firefox(options=options, executable_path=r'/Program Files/InstagramDownloader/geckodriver.exe')
                    ...
                    if True:
                        if True:
                            ...
                            try:
                                ...
                                browser.get('https://www.instasaveonline.com')
                                sleep(2)

                                inputs = browser.find_elements_by_tag_name('input')[0]

                                type_like_a_person(f'https://www.instagram.com/p/{post.shortcode}', inputs)

                                hide = browser.find_element_by_xpath('/html/body/div[4]/div/div/div[2]/div')
                                hide.click()

                                sleep(2)
                                button = browser.find_element_by_xpath('//*[@id="submit"]')
                                button.click()
                                sleep(20)

                                src = browser.page_source

                                soup = bs4(src, 'html.parser')
                                a = soup.find_all('a')

                                links = []
                                for tag_a in a:
                                    if tag_a.getText() == 'Download file':
                                        link = urllib.parse.unquote_plus(tag_a['href']).replace('download.php?url=', '')
                                        links.append(link)

                                print(links)
                                print(f'Downloading www.instagram.com/p/{post.shortcode}. It has {len(links)} parts.')
                                c = 1
                                for url in links:
                                    print(f'Downloading part {c}.')
                                    ext = '.mp4'
                                    filename = f'{post.owner_username}_{post_date}{ext}'
                                    r = requests.get(url, stream=True)
                                    with open(f'{os.path.join(HOME,"Posts/")}{c}{filename}', 'wb') as file:
